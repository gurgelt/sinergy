/**
 * js/orcamentos.js - Versão Final Completa (Corrigida)
 * Funcionalidades: Paginação, Dashboard, Validação, Assistente, PDF e CRUD.
 */
...
// Descrições para os Tooltips (Corrigido: Aspas duplas removidas da Soleira)
...
/**
 * Função inteligente para encontrar a descrição
 * Procura por palavras-chave parciais (ex: "Guia" encontra "Guia Lateral 70")
 */
function getItemDescription(nomeItem) {
    if (!nomeItem) return null;
    const nomeUpper = nomeItem.toUpperCase();

    // Percorre as chaves e vê se alguma delas está contida no nome do item
    for (const [chave, desc] of Object.entries(DESCRICOES_ITENS)) {
        if (nomeUpper.includes(chave.toUpperCase())) {
            return desc;
        }
    }
    return null;
...
const MOTORES_DISPONIVEIS = [
    ...
    { peso: 1500, alturaMax: 8.0, nome: 'AC 1500 (J) (220v ou 380v) CONJUNTO' },
    { peso: 2000, alturaMax: 8.0, nome: 'AC 2000 (J) (220v ou 380v) CONJUNTO' }
];
...
async function initializeApp() {
    ...
    // INICIALIZA O TOOLTIP GLOBAL
    initGlobalTooltip();
    setupEventListeners();
    
    const produtosPromise = loadProdutos();
    const orcamentosPromise = loadOrcamentos();
    const clientesPromise = loadClientes(); 
    loadVendedoresParaSelect();

    await Promise.all([produtosPromise, orcamentosPromise, clientesPromise]);
...
async function loadClientes() {
    try {
        ...
        if (res.ok) allClientes = await res.json();
...
async function loadVendedoresParaSelect() {
    try {
        ...
        if (response.ok) {
            ...
            if (currentUser && vendedores.includes(currentUser)) select.value = currentUser;
        }
    } catch (error) { console.error(error); }
}
...
function selecionarTipoKit(kitTipo) { assistenteKitTipo = kitTipo; fecharModalKit(); if (kitTipo === 'manual') abrirModal('create'); else if (kitTipo === 'automatica') abrirModalTipoMotor(); else { assistenteTipoMotor = null; abrirModalAssistenteCliente(); } }
function selecionarTipoMotor(tipo) { assistenteTipoMotor = tipo; fecharModalTipoMotor(); abrirModalAssistenteCliente(); }
...
function preencherKit(kitTipo) {
    ...
    // 1. Define Motor (se for kit automático)
    ...
    // 2. Define Tubo (Eixo) - ATUALIZADO (Regra de 4 metros)
    let tuboRecomendado = '';
    ...
    // 4. Define Antiqueda (Regra de 3 faixas)
    let antiqueda = '';
    ...
    // 5. Kit Automático - ATUALIZADO (Nome do Kit Central 99A)
    let kitAuto = [];
    ...
    // 6. Processa os itens e calcula preços
    itensAdd.forEach(nome => {
        ...
        if (prod) {
            ...
            if (prod.UnidadeMedida === 'm²' && prod.NomeItem === laminaRecomendada) {
                ...
                // ATUALIZADO: Cobra por metro linear (comp * preco)
                // Se não tiver comprimento definido (erro), usa 4m como fallback
                qtd = 1; 
...
function renderizarItensTemp() {
    ...
    ordenados.forEach(item => {
        ...
        if (descricao) {
            // Adiciona tanto o data-tooltip (CSS) quanto o title (Nativo do navegador)
            // O title garante que apareça mesmo se o CSS falhar ou for cortado
        ...
        tr.innerHTML = `
            ...
            <td class="text-center">
                <button type="button" class="action-button edit" onclick="iniciarEdicaoItem(${item.originalIndex})" title="Editar">
                    <i class="fas fa-edit"></i>
                </button>
                <button type="button" class="action-button delete" onclick="removerItemTemp(${item.originalIndex})" title="Excluir">
...
async function salvarOrcamento(event) {
    event.preventDefault(); const sel = document.getElementById('select-item-produto'); if (sel.value) if (!adicionarItem()) return; const btnSalvar = document.getElementById('btn-salvar'); if (itensTemp.length === 0) return showNotification('Adicione pelo menos um item ao orçamento.', 'warning'); btnSalvar.disabled = true; btnSalvar.innerHTML = 'Salvando...';
    const sub = itensTemp.reduce((a, b) => a + b.total, 0); const frete = parseFloat(document.getElementById('input-valor-frete').value) || 0; const desc = parseFloat(document.getElementById('input-desconto-geral').value) || 0; const total = (sub + frete) * (1 - desc / 100);
    const payload = { usuarioID: currentUserID, tipo: tipoOrcamentoAtual, status: (modalMode === 'edit' && orcamentoAtual) ? orcamentoAtual.Status : 'pendente', dataOrcamento: document.getElementById('input-data-orcamento').value, dataValidade: document.getElementById('input-data-validade').value, clienteNome: document.getElementById('input-cliente-nome').value, clienteDocumento: document.getElementById('input-cliente-documento').value, clienteEndereco: document.getElementById('input-cliente-endereco').value, clienteCidadeUF: document.getElementById('input-cliente-cidade-uf').value, clienteContato: document.getElementById('input-cliente-contato').value, clienteEmail: document.getElementById('input-cliente-email').value, itens: itensTemp, subtotal: sub, temFrete: document.getElementById('check-frete').checked, valorFrete: frete, descontoGeralPercent: desc, valorTotal: total, observacoes: document.getElementById('input-observacoes').value };
    try { let url = `${API_URL}/orcamentos`; let method = 'POST'; if (modalMode === 'edit' && orcamentoAtual) { url += `/${orcamentoAtual.ID}`; method = 'PUT'; } const res = await fetch(url, { method: method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); if (!res.ok) throw new Error('Falha ao salvar'); showNotification('Salvo com sucesso!', 'success'); fecharModal(); await loadOrcamentos(); renderizarTabela(); atualizarEstatisticas(); } catch (e) { showNotification(e.message, 'error'); } finally { btnSalvar.disabled = false; btnSalvar.textContent = 'Salvar'; }
}
...
async function atualizarStatusOrcamentoPorId(id, status) {
    if (status === 'aprovado') { let orc = orcamentoAtual || orcamentos.find(o => o.ID === id); if (orc) { const val = validarClienteParaAprovacao(orc); if (!val.valido) { abrirModalAvisoCliente(val.msg); return; } } }
    try { const res = await fetch(`${API_URL}/orcamentos/${id}`, { method: 'PUT', headers: {'Content-Type':'application/json'}, body: JSON.stringify({status}) }); if (!res.ok) throw new Error('Erro na requisição'); showNotification(`Status alterado!`, 'success'); fecharModalView(); await loadOrcamentos(); renderizarTabela(); atualizarEstatisticas(); } catch (e) { showNotification(e.message, 'error'); }
}
...
function getEnderecosExtrasDoFormulario() { const e = []; document.querySelectorAll('.endereco-extra-row').forEach(r => { const t = r.querySelector('.extra-tipo').value; const ad = r.querySelector('.extra-end').value.trim(); const ci = r.querySelector('.extra-cid').value.trim(); if (ad) e.push({ tipo, endereco, cidade }); }); return e; }
...
function filterAndShowClienteDropdown() {
    ...
    // Se tiver termo, filtra a lista completa
    let fil = [];
    if (term.length === 0) {
        fil = allClientes.slice(0, 20); // Limite para não travar a tela se tiver muitos
    } else {
...
function filterAndShowAssistenteClienteDropdown() {
    ...
    // Mesma lógica: mostra top 20 se vazio
    let fil = [];
    if (term.length === 0) {
...
function initGlobalTooltip() {
    // 1. Cria o elemento do tooltip no corpo da página se não existir
    ...
    // 2. Variáveis para controle de posição
    const spacing = 10; // Espaço entre o ícone e o balão

    // 3. Evento GLOBAL: Detecta mouse sobre qualquer .info-icon
    document.body.addEventListener('mouseover', (e) => {
        if (e.target.classList.contains('info-icon')) {
            ...
            // Cálculos de Posição Matemática
            ...
            // Correção: Se sair pelo topo da tela, joga para baixo
            if (top < 0) {
                top = iconRect.bottom + spacing;
                // (Opcional) Inverter a seta via CSS se quiser perfeccionismo
            }

            // Correção: Se sair pela esquerda
            if (left < 10) left = 10;
            
            // Correção: Se sair pela direita
    ...
    // 4. Evento GLOBAL: Esconde ao sair
...
function preencherFormulario(orcamento) {
    ...
    // 1. Mapeia os itens do banco para o formato temporário do front
    ...
    // 5. Novos Campos de Pagamento (Se existirem no HTML)
    ...
    // 6. Bloqueia edição do nome do cliente (para não perder o vínculo do ID)
